...
const StakeAction: React.FC<StakingCardActionsProps> = ({
  stakedBalance,
  tokenBalance,
  tokenName,
  pid,
  ...
  const stakingStakedPrice = useStakingStakedPrice(pid)
  ...
  const renderStakingButtons = () => {
    return rawStakedBalance === 0 ? (
      <Button onClick={onPresentDeposit}>{TranslateString(385, 'Stake')}</Button>
    ) : (
      <IconButtonWrapper>
  ...
  return (
    <Flex justifyContent="space-between" alignItems="center">
      <HeadingLeft color={rawStakedBalance === 0 ? 'textDisabled' : 'text'}>
        {displayBalance}
        <CardBusdValue value={stakingStakedPrice.stakingStakedPrice}/>
      </HeadingLeft>
