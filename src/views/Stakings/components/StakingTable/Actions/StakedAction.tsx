...
const Staked: React.FunctionComponent<StakingWithStakedValue> = ({ pid, stakingToken, rewardToken }) => {
  const TranslateString = useI18n()
  const { account } = useWeb3React()
  const [requestedApproval, setRequestedApproval] = useState(false)
  const { allowance, tokenBalance, stakedBalance } = useStakingUser(pid)
  const { onStake } = useStake(pid)
  const { onUnstake } = useUnstake(pid)
  const web3 = useWeb3()
  const lpSymbol = stakingToken.symbol
  const lpAddresses = stakingToken.address
